...
public class TrackpadFragment extends Fragment implements OnTouchListener, OnCheckedChangeListener{
...
//	private static final String DEBUG_TAG = "Gestures";
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(R.layout.fragment_trackpad, container, false);
		mPadView = (GestureOverlayView) ((RelativeLayout) rootView).getChildAt(0);
		mPadView.setOnTouchListener(this);
		
		mCustomGestureButton = (ToggleButton) ((RelativeLayout) rootView).getChildAt(1);
		mCustomGestureButton.setOnCheckedChangeListener(this);
		
		mStoreFile = new File(getActivity().getApplicationContext().getFilesDir(), "gestures");
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		( (MainActivity) getActivity()).loadGestures();
	}
	...
	public boolean onTouch(View v, MotionEvent event) {
        ...
        if(event.getPointerCount()==1){
        	...
        	case MotionEvent.ACTION_MOVE:
        		...
        		if(/*!isLongPressed && */Math.abs(newpos[0][0]-oldpos[0][0])+Math.abs(newpos[0][1]-oldpos[0][1])>4)
        			//Finger sliding by relative position (newpos[0][0]-oldpos[0][0]) in X and (newpos[0][1]-oldpos[0][1]) in Y
        			//text2.setText("Movement by "+(newpos[0][0]-oldpos[0][0])+","+(newpos[0][1]-oldpos[0][1]));
        			SocketSetup.print("mouse move " + (newpos[0][0]-oldpos[0][0]) + " " + (newpos[0][1]-oldpos[0][1]));
        	...
        	case MotionEvent.ACTION_POINTER_DOWN:
        		...
        		oldpos[0][0] = event.getX(0); oldpos[1][0] = event.getX(1);
        		oldpos[0][1] = event.getY(0); oldpos[1][1] = event.getY(1);
        		old_distance = FloatMath.sqrt((oldpos[1][0]-oldpos[0][0])*(oldpos[1][0]-oldpos[0][0]) + (oldpos[1][1]-oldpos[0][1])* (oldpos[1][1]-oldpos[0][1]));
        		//text1.setText("Double touch!");
        		break;
        	...
        	case MotionEvent.ACTION_UP:
        		break;
        	case MotionEvent.ACTION_MOVE:
        		newpos[0][0] = event.getX(0); newpos[1][0] = event.getX(1);
        		newpos[0][1] = event.getY(0); newpos[1][1] = event.getY(1);
        		old_distance = FloatMath.sqrt((oldpos[1][0]-oldpos[0][0])*(oldpos[1][0]-oldpos[0][0]) + (oldpos[1][1]-oldpos[0][1])* (oldpos[1][1]-oldpos[0][1]));
        		new_distance = FloatMath.sqrt((newpos[1][0]-newpos[0][0])*(newpos[1][0]-newpos[0][0]) + (newpos[1][1]-newpos[0][1])* (newpos[1][1]-newpos[0][1]));
        		float dist_diff = new_distance-old_distance;
        		//text2.setText("Distance ratio is "+dist_diff);
        		if(dist_diff<-1)
        			if(!isZoomedin) {
        				isZoomedin = true;
        				SocketSetup.print("Zoom in");
        			}
        			//Fingers moving apart - zoom in
        			//text1.setText("Zoom In!");
        		else if(dist_diff>1)
        			if(!isZoomedout) {
        				isZoomedout = true;
        				SocketSetup.print("Zoom out");
        			}
        			//Fingers converging - zoom out
        			//text1.setText("Zoom Out!");
        		for(int i=0;i<=1;i++)
        			for(int j=0;j<=1;j++)
	...
	class MyGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent event1, MotionEvent event2, 
            ...
            //This is only imp if we want to know the x,y when sliding started and when it ended
